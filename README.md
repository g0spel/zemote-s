...
Zflow 在手机上管理你的桌面 ZCode:扫码添加设备 → 连接 → 任务对话、自动化定时任务、会话洞察、模型供应商与用量管理。整个应用围绕自研的 **Ember 设计语言**构建——暖炭色板、更纱黑体、"内容区安静、控制区紧凑"的分区原则;协议层为纯 Dart 复刻,280+ 测试覆盖,全部关键改动经逐任务代码审查落地。
...
| 对话页(四态:基础 / 计划模式 / 会话抽屉 / 洞察 sheet) | 自动化 · 设置 · 二级页 |
|---|---|
| <img src="docs/screenshots/chat-hifi.png" alt="对话页"> | <img src="docs/screenshots/pages-hifi.png" alt="管理页"> |

深色为设计基准主题,浅色(暖纸白)同步推导;设计定稿见[设计规范](docs/superpowers/specs/2026-08-28-ui-redesign-design.md)。
...
- **流式回复**:逐 token 渲染;思考过程与工具调用按真实顺序穿插(思考 → 文本 → 工具 → 文本),默认展开可收起,左侧状态 rail 标示运行态(蓝运行 / 绿成功 / 红失败);工具输出、diff、内嵌图片直接可读。
- **消息状态机**:点发送气泡立即上屏,角标演进「发送中 → 已发送 → 处理中」;回合完成角标退场;发送失败消息常驻,点击重发(自动重建会话 / 重传未上传附件 / held 队列重新确认)。
- **模型 pill 与四模式**:顶栏 `模型名 | 思考强度` + 模式徽——计划(橙)/ 编辑(蓝)/ YOLO(红),构建模式隐藏;点开面板 模式 → 思考强度 → 模型 三分区即时切换,档位随桌面端下发动态;计划 / YOLO 模式在消息流顶部常驻状态条,风险可见性分级。
- **会话抽屉**:左缘滑出——工作区条(点击底部升起切换层)、本地搜索、＋新会话、置顶 / 今天 / 更早分组(listPinnedTasks 实时驱动)、运行蓝点 / 等待输入黄点、多选批量管理(置顶 / 归档 / 删除);列表带离线缓存,弱网秒开。
- **完整输入能力**:排队消息(held queue,自动 / 手动放行)、目标指令(Goal)、图片与文件附件(384KB 分片上传带进度)、斜杠命令 + 桌面端 Skills(`$` 前缀,带选择弹层)。
- **交互应答**:AI 权限请求与多问题表单按官方契约渲染与一次性提交(`action:accept` + `answers`),支持部分作答;权限三选项、自由输入、单选多选表单全覆盖。
- **辅助能力**:侧对话(side chat)并行提问、回复点赞点踩、回合级文件变更查看与回滚(rewind 带预览)。

### 会话洞察

对话页输入区上方把手上滑呼出(62% 起,可拉全屏;后台有任务时把手浮出计数徽),三面板:

- **待办**:镜像桌面宿主的 TodoWrite 推导 + 计划模式进度条(快照 plan.items);
- **文件**:回合摘要(横滑)+ 文件级增删明细,点击进全屏 diff 查看器;每个回合完成后自动预加载最新 diff,协议层内置 stale 自愈重试;
- **后台**:后台任务与子代理混排时间线——运行中转圈、待取结果、成功 / 失败 / 取消终态,子代理显示摘要与起止时间,已结束子代理逐条列出。
...
- 用量页:MCP 主额度大卡(剩余 / 总量 + 进度 + 重置时间)、五小时窗口与每周配额双卡(≤20% 变黄、≤10% 变红)、订阅到期与套餐信息(本地时区渲染)。
...
任务运行中 Android 前台服务常驻并实时更新进展;完成 / 失败 / 中断终态区分的静默提醒;**待处理交互提醒**(权限请求 / AI 提问卡住时,每个交互只提醒一次);点击直达对应对话。
...
本项目是对 ZCode 远程控制协议的独立复刻,自 v0.3.5 起以自有路线迭代:安全加固(强制 TLS / 凭据加密 / 更新校验)、自动化管理、会话洞察面板、连接可靠性打磨(三级自愈 / 锁屏无感恢复)、消息状态机,v0.10.0 起全面落地的 Ember 设计语言,v0.11.0 更名 Zflow,以及 v1.0.0 的真机全量稳定化。协议演进经宿主源码核对与实机探针验证后跟进,不自动跟随任何外部实现。
