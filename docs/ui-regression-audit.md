...
- [ ] T1(C1)session_drawer 数据管线回退旧法骨架:任务为主体+索引富化;叠加 F9/F10(用户拍板后执行)
- [ ] T2(C2)刷新触发器随 T1 简化(保留开抽屉重拉+活跃条目消失重拉;去掉 15s 节流)
- [ ] T3(C5)恢复丢失操作:重命名/取消归档/查看原始快照(用户拍板后执行)
- [ ] T4(C5)左滑快捷操作是否恢复(用户拍板)
- [ ] T5(B1)AppSession 在途守卫:保留或回退(用户拍板;默认保留)
- [ ] T6(A5/B4)诊断日志(debugPrint 双写)是否加开关或降噪(用户拍板;默认保留)
- [ ] T7 全量测试 + release 构建 + 真机回归(归档分组/混入/桌面归档同步/首发链路/思考档)
